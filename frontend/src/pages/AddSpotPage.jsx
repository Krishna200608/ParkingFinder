...
const AddSpotPage = () => {
	...
	// 🧭 My Location button handler (FIXED)
	const handleMyLocation = async () => {
		...
		if (!window.isSecureContext) {
			toast.error("Geolocation requires a secure context (HTTPS)");
			return;
		}
		...
		navigator.geolocation.getCurrentPosition(
			async (pos) => {
				...
				// Smooth fly and update map visuals
				if (mapRef.current) {
			...
			<div className="max-w-7xl mx-auto px-6 mt-10 grid md:grid-cols-2 gap-10">
				...
				<div className="bg-white rounded-xl shadow-lg overflow-hidden relative border border-gray-100">
					...
					<button
						...
						className="absolute top-25 left-15 z-[1000] bg-blue-600 hover:bg-blue-700 text-white 
             px-4 py-2 rounded-md shadow-md text-sm flex items-center gap-2 transition-all"
					>
